...
# my package is called MyPackage
load_all("MyPackage")
```
...
```{r, include= T}
est <- read_csv("MyPackage/data/est-2.csv")
...
dat <- read.csv("MyPackage/data/est-2.csv")
data <- read.csv("MyPackage/data/contraceptive_use-2.csv")
devtools::load_all("MyPackage")
plot_cp(data, est %>% select(-iso), iso_code = 4)
plot_cp(data %>% select(-contraceptive_use_modern), est %>% select(-Year), iso_code = 4)
plot_cp(data %>% select(-contraceptive_use_modern), est %>% (-Year), iso_code = 4)
```
...
```{r, error = T, include = T}
dat_bug <- dat %>%
  mutate("contraceptive_use_modern" = FALSE)
...
https://github.com/hfeliz03/BIOSTATS632_HW3
